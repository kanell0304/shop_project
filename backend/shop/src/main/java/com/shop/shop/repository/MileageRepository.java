...
public interface MileageRepository extends JpaRepository<Mileage, Long> {
    ...
    @Query("SELECT m FROM Mileage m WHERE m.member.id = :memberId")
    public Page<List<MileageDTO>> findAllByMemberId(Pageable pageable, @Param("memberId") Long memberId);
    ...
    @Query("SELECT m FROM Mileage m WHERE m.order.id = :orderId")
    public Page<List<MileageDTO>> findAllByOrderId(Pageable pageable, @Param("orderId") Long orderId);
    ...
    @Query("SELECT m FROM Mileage m WHERE m.mileageDate BETWEEN :mileageDate1 AND :mileageDate2")
    public Page<List<MileageDTO>> findByDuringPeriod(Pageable pageable, @Param("mileageDate1") LocalDateTime mileageDate1, @Param("mileageDate2") LocalDateTime mileageDate2);
    ...
    @Query("SELECT m FROM Mileage m WHERE m.member.id = :memberId AND m.mileageDate BETWEEN :mileageDate1 AND :mileageDate2")
    public Page<List<MileageDTO>> findByDuringPeriodFromMemberId(Pageable pageable, @Param("memberId") Long memberId, @Param("mileageDate1") LocalDateTime mileageDate1, @Param("mileageDate2") LocalDateTime mileageDate2);
